...
class CoreListener extends Listener_1.Listener {
    ...
    async run(payload) {
        ...
        if (globalResult.isErr()) {
            this.container.client.emit(Events_1.Events.ContextMenuCommandDenied, globalResult.unwrapErr(), payload);
            return;
        }
        // Run command-specific preconditions:
        const localResult = await command.preconditions.contextMenuRun(interaction, command, payload);
        if (localResult.isErr()) {
            this.container.client.emit(Events_1.Events.ContextMenuCommandDenied, localResult.unwrapErr(), payload);
            return;
        }
