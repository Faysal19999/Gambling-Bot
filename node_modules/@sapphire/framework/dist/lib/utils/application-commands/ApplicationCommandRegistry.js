...
class ApplicationCommandRegistry {
    ...
    registerChatInputCommand(command, options) {
        ...
        if (!(0, utilities_1.isNullishOrEmpty)(options?.guildIds)) {
            for (const id of options.guildIds) {
                this.guildIdsToFetch.add(id);
                ApplicationCommandRegistries_1.allGuildIdsToFetchCommandsFor.add(id);
            }
        }
        return this;
    }
    ...
    async handleCommandPresent(applicationCommand, apiData, behaviorIfNotEqual, guildId) {
        let differences = [];
        if (behaviorIfNotEqual === Enums_1.RegisterBehavior.VerboseOverwrite) {
            const now = Date.now();
            // Step 0: compute differences
            differences = (0, computeDifferences_1.getCommandDifferences)((0, normalizeInputs_1.convertApplicationCommandToApiData)(applicationCommand), apiData, guildId !== null);
        ...
        if (behaviorIfNotEqual === Enums_1.RegisterBehavior.Overwrite || behaviorIfNotEqual === Enums_1.RegisterBehavior.LogToConsole) {
            const now = Date.now();
            // Step 0: compute differences
            const areThereDifferences = (0, computeDifferences_1.getCommandDifferencesFast)((0, normalizeInputs_1.convertApplicationCommandToApiData)(applicationCommand), apiData, guildId !== null);
            const later = Date.now() - now;
        ...
        this.logCommandDifferencesFound(applicationCommand, behaviorIfNotEqual === Enums_1.RegisterBehavior.LogToConsole, differences);
        // Step 2: if the behavior is to log to console, only log the differences
        if (behaviorIfNotEqual === Enums_1.RegisterBehavior.LogToConsole) {
            return;
        }
