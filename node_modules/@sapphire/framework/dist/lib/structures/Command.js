...
class Command extends pieces_1.AliasPiece {
    ...
    messagePreParse(message, parameters, context) {
        const parser = new lexure_1.Parser(this.strategy);
        const args = new lexure_1.ArgumentStream(parser.run(this.lexer.run(parameters)));
    ...
    async reload() {
        ...
        const { applicationCommands, globalCommands, guildCommands } = await (0, getNeededParameters_1.getNeededRegistryParameters)(updatedRegistry.guildIdsToFetch);
    ...
    parseConstructorPreConditionsCooldown(options) {
        ...
        if (limit && delay) {
            const scope = options.cooldownScope ?? defaultCooldown?.scope ?? Enums_1.BucketScope.User;
