...
class InteractionHandlerStore extends pieces_1.Store {
    ...
    async run(interaction) {
        ...
        for (const handler of this.values()) {
            ...
            const result = await result_1.Result.fromAsync(() => handler.parse(interaction));
            result.match({
                ok: (option) => {
                    ...
                    option.inspect((value) => {
                        ...
                        const promise = result_1.Result.fromAsync(() => handler.run(interaction, value)) //
                ...
                err: (error) => {
                    ...
                    this.container.client.emit(Events_1.Events.InteractionHandlerParseError, error, { interaction, handler });
                }
            });
        }
        ...
        for (const result of results) {
            ...
            res.inspectErr((value) => this.container.client.emit(Events_1.Events.InteractionHandlerError, value.error, { interaction, handler: value.handler }));
        }
        return true;
    }
}
