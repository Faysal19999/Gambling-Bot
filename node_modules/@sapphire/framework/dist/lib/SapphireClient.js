...
class SapphireClient extends discord_js_1.Client {
    constructor(options) {
        ...
        for (const plugin of SapphireClient.plugins.values(Enums_1.PluginHook.PreGenericsInitialization)) {
            plugin.hook.call(this, options);
            this.emit(Events_1.Events.PluginLoaded, plugin.type, plugin.name);
        }
        this.logger = options.logger?.instance ?? new Logger_1.Logger(options.logger?.level ?? 30 /* LogLevel.Info */);
        pieces_1.container.logger = this.logger;
        if (options.enableLoaderTraceLoggings ?? pieces_1.container.logger.has(10 /* LogLevel.Trace */)) {
        ...
        for (const plugin of SapphireClient.plugins.values(Enums_1.PluginHook.PreInitialization)) {
            plugin.hook.call(this, options);
            this.emit(Events_1.Events.PluginLoaded, plugin.type, plugin.name);
        }
        ...
        for (const plugin of SapphireClient.plugins.values(Enums_1.PluginHook.PostInitialization)) {
            plugin.hook.call(this, options);
            this.emit(Events_1.Events.PluginLoaded, plugin.type, plugin.name);
        }
    }
    ...
    async login(token) {
        ...
        for (const plugin of SapphireClient.plugins.values(Enums_1.PluginHook.PreLogin)) {
        ...
        for (const plugin of SapphireClient.plugins.values(Enums_1.PluginHook.PostLogin)) {
            await plugin.hook.call(this, this.options);
            this.emit(Events_1.Events.PluginLoaded, plugin.type, plugin.name);
        }
        return login;
